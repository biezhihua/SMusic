...
GLuint OpenGLUtils::createProgram(const char *vertexShader, const char *fragShader) {
    ...
    if (!linked) {
        ...
        if (infoLen > 1) {
            ...
            ALOGE("OpenGLUtils", "[%s] Error linking program: %s", __func__, infoLog);
            // 使用完成后需要释放字符串分配的内存
            free(infoLog);
        }
        // 删除着色器释放内存
...
GLuint OpenGLUtils::loadShader(GLenum type, const char *shaderSrc) {
    ...
    if (!compiled) {
        ...
        if (infoLen > 1) {
            ...
            ALOGE("OpenGLUtils", "[%s] Error compiling shader: %s", __func__, infoLog);
            // 使用完成后需要释放字符串分配的内存
            free(infoLog);
        }
        // 删除编译出错的着色器释放内存
...
void OpenGLUtils::checkActiveUniform(GLuint program) {
    ...
    for (int i = 0; i < numUniforms; ++i) {
        ...
        glGetActiveUniform(program, static_cast<GLuint>(i), maxLen, nullptr, &size, &type,
                           uniformName);
...
void OpenGLUtils::bindTexture(int location, int texture, int index, int textureType) {
    ...
    glBindTexture(static_cast<GLenum>(textureType), static_cast<GLuint>(texture));
